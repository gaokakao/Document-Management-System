	...
	if (strlen($error) == 0)
	{
		...
		switch ($change)
		{
			...
			case '1':
				...
				$uploadfile = 'documents/'.$_SESSION['id'].'.'.substr($dokumentas['name'], strlen($dokumentas['name']) - 3, 3);
		...
		$query = "UPDATE sutartys SET numeris = '".$numeris."', kitas_numeris = '".$kitas_numeris."', kita_salis = ".$kita_salis.", data = '".$data."', terminas = '".$terminas."', objektas = '".$objektas."', tipas = '".$tipas."', pobudis = '".$pobudis."', strukturinis_objektas = ".$strukturinis_objektas.", atsakingas_asmuo = ".$atsakingas_asmuo.", pasirases_asmuo = ".$pasirases_asmuo.", galiojimas = '".$galiojimas."', komentaras = '".$komentaras."'";
		if (is_null($uploadfile))
			$query .= ", dokumentas = NULL";
		else
			$query .= ", dokumentas = '".$uploadfile."'";
		$query .= " WHERE sutarties_id = ".$_SESSION['id'];
		$database->Query($query);

		$query = "UPDATE sutartys SET kita_salis = ".$kita_salis." WHERE parent_id = ".$_SESSION['id'];
		$database->Query($query);

		unset($_SESSION['action']);

		$_SESSION['edit_id'] = $_SESSION['id'];
...
{
	$query = "SELECT * FROM sutartys WHERE sutarties_id = ".$_SESSION['id'];
...
<tr bgcolor="#DDDDDD">
	<td colspan="4"><b>Sutarties informacijos keitimas</b></td>
</tr>
...
{
	?>
	<input type="hidden" name="sutartis" value="<?= $sutartis; ?>">
	<input type="hidden" name="priedas" value="<?= $priedas; ?>">
	<input type="hidden" name="numeris" value="<?= $numeris; ?>">
	<input type="hidden" name="kitas_numeris" value="<?= $kitas_numeris; ?>">
	<input type="hidden" name="kita_salis" value="<?= $kita_salis; ?>">
...
function resetForm()
{
	document.sutartys.priedas.selectedIndex = 0;
