...
{
	$sutartis = $_POST['sutartis'];
	$priedas = $_POST['priedas'];
	$numeris = $_POST['numeris'];
	$kitas_numeris = $_POST['kitas_numeris'];
	$kita_salis = $_POST['kita_salis'];
	$data = $_POST['data'];
	...
	$error = '';
	if (($sutartis == '1') and ($priedas == 'NULL'))
	{
		$error .= 'Nenurodyta sutartis, kuriai priklauso šis priedas<br>';
		$errors['sutartis'] = 1;
	}
	if ($numeris == '')
	{
	...
	if (($sutartis == '0') and ($kita_salis == 'NULL'))
	{
		$error .= 'Nepasirinkta kita šalis<br>';
		$errors['kita_salis'] = 1;
	}
	if ($sutartis == '1')
	{
		$query = "SELECT kita_salis FROM sutartys WHERE sutarties_id = ".$priedas;
	...
	if (strlen($error) == 0)
	{
		...
		$priedas = (($sutartis == '0') ? 'NULL' : $priedas);
...
<table width="650" cellpadding="4" cellspacing="1" bgcolor="#AAAAAA">
...
<tr bgcolor="#DDDDDD">
	<td colspan="4"><b>Nauja sutartis</b></td>
</tr>
<tr bgcolor="#FFFFFF">
	...
	<td colspan="2">
		...
		<tr>
			<td><input <? if ($sutartis == '0') echo 'checked'; ?> class="checkbox" type="radio" name="sutartis" value="0" onclick="element = document.getElementById('parent_id'); element.style.display = 'none'; element = document.getElementById('kita_salis'); element.style.display = 'block'"></td>
			<td>Nauja sutartis</td>
			<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
			<td><input <? if ($sutartis == '1') echo 'checked'; ?> class="checkbox" type="radio" name="sutartis" value="1" onclick="element = document.getElementById('parent_id'); element.style.display = 'block'; element = document.getElementById('kita_salis'); element.style.display = 'none'"></td>
			<td>Kitos sutarties priedas ar papildymas</td>
			<td>&nbsp;</td>
		</tr>
		</table>
	</td>
</tr>
<tr bgcolor="#FFFFFF" id="parent_id" style="<? if ($sutartis == '0') echo 'display: none;'; ?>">
	<td nowrap align="right">
		<span <? if ($errors['sutartis'] == 1) echo 'style="color: #FF0000"';?>>
			Šis priedas papildo sutartį:
		</span>
	</td>
	<td colspan="2">
		<select name="priedas">
			<option <? if ($priedas == 'NULL') echo 'selected' ?> value="NULL">---</option>
			<?
				$query = "SELECT sutartys.sutarties_id, sutartys.numeris, subjektai.pavadinimas FROM sutartys";
				$query .= " LEFT JOIN subjektai ON sutartys.kita_salis = subjektai.subjekto_id ";
				$query .= " WHERE sutartys.parent_id IS NULL ORDER by subjektai.pavadinimas, sutartys.numeris ASC";
				...
				for ($i = 0; $i < mysqli_num_rows($result); $i++)
				{
					...
					?>
					<option <? if ($priedas == $row['sutarties_id']) echo 'selected' ?> value="<?= $row['sutarties_id']; ?>"><?= $row['pavadinimas']; ?> [<?= $row['numeris']; ?>]</option>
					<?
				}
			?>
		</select>
...
<tr bgcolor="#FFFFFF" id="kita_salis">
	<td nowrap align="right">
		<span <? if ($errors['kita_salis'] == 1) echo 'style="color: #FF0000"';?>>
	...
	<td colspan="2">
		<select name="kita_salis">
			...
			<?
				$query = "SELECT * FROM subjektai ORDER BY pavadinimas ASC";
				$result = $database->Query($query);
