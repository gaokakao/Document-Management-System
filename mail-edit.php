...
$query = "SELECT * FROM sutartys WHERE sutarties_id = ".$_SESSION['edit_id'];
...
<tr bgcolor="#DDDDDD">
	<td colspan="2">
		...
		{
			echo 'Sutarčių registre pakeista sutarties Nr. '.$row['numeris'].' informacija';
		}
		else
		{
			$query = "SELECT numeris FROM sutartys WHERE sutarties_id = ".$row['parent_id'];
			$parent = $database->QueryAssoc($query);
			echo 'Sutarčių registre pakeista sutarties Nr. '.$parent['numeris'].' priedo Nr. '.$row['numeris'].' informacija';
		}
		?>
		</b>
	</td>
...
if (is_null($row['parent_id']))
	$mail_subject = 'Pranešimas apie sutarties Nr. '.$row['numeris'].$objektas.' informacijos pakeitimą';
else
	$mail_subject = 'Pranešimas apie sutarties Nr. '.$parent['numeris'].' priedo Nr. '.$row['numeris'].$objektas.' informacijos pakeitimą';
